...
/* ─── Black laurel wreath (same paths, light-theme fill) ─── */
function LaurelWreathBlack() {
  return (
    <svg
      viewBox="0 0 892.77 688.08"
      fill={BLACK}
      style={{ width: '100%', maxWidth: '680px', display: 'block' }}
      aria-hidden="true"
    >
...
/* ─── Icons (black strokes for light background) ─── */
function IconUNGC({ size = 28 }: { size?: number }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" width={size} height={size}>
      <circle cx="16" cy="16" r="13" stroke={BLACK} strokeWidth="1.7"/>
      <ellipse cx="16" cy="16" rx="6" ry="13" stroke={BLACK} strokeWidth="1.4" fill="none"/>
      <line x1="3" y1="16" x2="29" y2="16" stroke={BLACK} strokeWidth="1.4"/>
      <path d="M4 10 Q16 14 28 10" stroke={BLACK} strokeWidth="1.1" fill="none"/>
      <path d="M4 22 Q16 18 28 22" stroke={BLACK} strokeWidth="1.1" fill="none"/>
    </svg>
  )
}
function IconWEPs({ size = 26 }: { size?: number }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" width={size} height={size}>
      <circle cx="16" cy="11" r="7.5" stroke={BLACK} strokeWidth="1.7"/>
      <line x1="16" y1="18.5" x2="16" y2="29" stroke={BLACK} strokeWidth="1.7"/>
      <line x1="11" y1="24" x2="21" y2="24" stroke={BLACK} strokeWidth="1.7"/>
      <circle cx="11" cy="7" r="1.4" fill={GOLD}/>
      <circle cx="21" cy="7" r="1.4" fill={GOLD}/>
      <circle cx="16" cy="4" r="1.4" fill={GOLD}/>
    </svg>
  )
}
function IconALC({ size = 28 }: { size?: number }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" width={size} height={size}>
      {/* chain links representing logistics chain */}
...
function IconHACCP({ size = 28 }: { size?: number }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" width={size} height={size}>
      <path d="M16 2L3 7V17C3 24.5 9 30 16 32C23 30 29 24.5 29 17V7L16 2Z"
        fill={`${GOLD}14`} stroke={BLACK} strokeWidth="1.7" strokeLinejoin="round"/>
      <path d="M10 16.5L14 20.5L22 11.5"
        stroke={BLACK} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}
function IconCoR({ size = 28 }: { size?: number }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" width={size} height={size}>
      {/* three nodes */}
      <circle cx="16" cy="5"  r="3" stroke={BLACK} strokeWidth="1.6"/>
      <circle cx="5"  cy="25" r="3" stroke={BLACK} strokeWidth="1.6"/>
      <circle cx="27" cy="25" r="3" stroke={BLACK} strokeWidth="1.6"/>
      {/* links */}
      <line x1="13.5" y1="7.5"  x2="7.2"  y2="22.5" stroke={BLACK} strokeWidth="1.4" strokeLinecap="round"/>
      <line x1="18.5" y1="7.5"  x2="24.8" y2="22.5" stroke={BLACK} strokeWidth="1.4" strokeLinecap="round"/>
      <line x1="8"    y1="25"   x2="24"   y2="25"   stroke={BLACK} strokeWidth="1.4" strokeLinecap="round"/>
      {/* centre shield accent */}
      <path d="M16 12 L19 15 V19 C19 20.7 17.7 22 16 22.5 C14.3 22 13 20.7 13 19 V15 Z"
        fill={`${GOLD}18`} stroke={BLACK} strokeWidth="1.3" strokeLinejoin="round"/>
    </svg>
  )
}

const credentials = [
  { label: 'UN Global Compact',              sub: 'Member',              desc: 'Committed to universal principles on human rights, labour standards, environmental responsibility and anti-corruption.', Icon: IconUNGC,    shortLabel: 'UN Global' },
  { label: "Women's Empowerment Principles", sub: 'WEPs Signatory',      desc: "Advancing gender equality and women's empowerment in the workplace, marketplace and community.",                          Icon: IconWEPs,    shortLabel: 'WEPs'      },
  { label: 'Australian Logistics Council',   sub: 'Industry Member',     desc: "Member of Australia's national peak body for the freight, logistics and supply chain industry, shaping best-practice policy and operations.", Icon: IconALC, shortLabel: 'ALC' },
  { label: 'HACCP',                          sub: 'Compliant Operations', desc: 'Hazard analysis and critical control point compliant - ensuring food-grade freight handling across every facility.',      Icon: IconHACCP,   shortLabel: 'HACCP'     },
  { label: 'Chain of Responsibility',         sub: 'CoR Compliant',        desc: "Fully compliant with Australia's Heavy Vehicle National Law - every party in our supply chain shares accountability for safety and legal obligations.", Icon: IconCoR, shortLabel: 'CoR' },
]

function TimerBar({ duration }: { duration: number }) {
  return (
    <div style={{ width: '100%', height: 2, background: `${GOLD}20`, borderRadius: 2, overflow: 'hidden' }}>
      <motion.div
        initial={{ width: '0%' }}
        animate={{ width: '100%' }}
        transition={{ duration: duration / 1000, ease: 'linear' }}
        style={{ height: '100%', background: `linear-gradient(90deg, ${GOLD}, ${GOLD_LIGHT})`, borderRadius: 2 }}
      />
    </div>
  )
}

export function Credentials3() {
  ...
  return (
    <section style={{ background: LIGHT, borderTop: '1px solid rgba(0,0,0,0.07)', overflow: 'hidden' }}>
      ...
      <motion.div
        ...
        style={{
          textAlign: 'center',
          padding: 'clamp(64px,8vw,110px) clamp(24px,5vw,80px) 0',
        }}
      >
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          <div style={{ width: 32, height: 1, background: `${GOLD}80` }} />
          <span style={{ fontSize: 10, fontWeight: 800, color: GOLD, letterSpacing: '3px', textTransform: 'uppercase' }}>
            Our Accreditations
          </span>
          <div style={{ width: 32, height: 1, background: `${GOLD}80` }} />
        </div>

        <h2 style={{ fontSize: 'clamp(30px,4vw,58px)', fontWeight: 900, color: BLACK, letterSpacing: '-0.04em', lineHeight: 1.08, margin: '0 0 6px' }}>
          Proven Expertise.
        </h2>
        <h2 style={{
          fontSize: 'clamp(30px,4vw,58px)', fontWeight: 900,
      ...
      <div style={{
        ...
        {/* Subtle warm glow behind wreath */}
        <div style={{
          position: 'absolute',
          ...
          <div style={{
            ...
            <AnimatePresence mode="wait" custom={dir}>
              <motion.div
                ...
                custom={dir}
                variants={{
                  enter:  (d: number) => ({ opacity: 0, y: d * 28, scale: 0.96 }),
                  center: { opacity: 1, y: 0, scale: 1 },
                  exit:   (d: number) => ({ opacity: 0, y: -d * 28, scale: 0.96 }),
                }}
                initial="enter"
                animate="center"
                exit="exit"
              ...
              >
                {/* Counter */}
                <div style={{ fontSize: 9, fontWeight: 700, color: `${GOLD}80`, letterSpacing: '2.5px', textTransform: 'uppercase' }}>
                  {active + 1} / {credentials.length}
                </div>
                ...
                {/* Description */}
                <p style={{ fontSize: 'clamp(9px,0.85vw,11.5px)', color: '#888', lineHeight: 1.75, margin: 0 }}>
                  {c.desc}
                </p>
              </motion.div>
            </AnimatePresence>
      ...
      <div style={{
        ...
        padding: 'clamp(28px,4vw,48px) clamp(16px,4vw,48px) 0',
        flexWrap: 'wrap',
      }}>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                borderRadius: 100,
                border: `1.5px solid ${isActive ? GOLD : 'rgba(0,0,0,0.12)'}`,
            ...
            >
              ...
              <span style={{
                ...
                color: isActive ? GOLD : 'rgba(0,0,0,0.35)',
      ...
      {/* ── Timer bar ── */}
      <div style={{ maxWidth: 360, margin: '20px auto 0', padding: '0 clamp(24px,5vw,80px)' }}>
        <TimerBar key={tick} duration={AUTO_MS} />
      </div>
